...
export const useNotificationStore = defineStore("notification-store", {
  ...
  actions: {
    ...
    notifyInfo(msg: string, timeout = defaultTimeout) {
      this.notify({ text: msg, color: "info", timeout: timeout });
    },
    notifyWarning(msg: string, timeout = defaultTimeout) {
      this.notify({ text: msg, color: "warning", timeout: timeout });
    },
    /**
